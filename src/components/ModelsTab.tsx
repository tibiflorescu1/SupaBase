...
export default function ModelsTab({ data, onUpdateData }: ModelsTabProps) {
  ...
  const handleSaveVehicle = () => {
    if (editingVehicle && editingVehicle.producator && editingVehicle.model && editingVehicle.categorieId) {
      ...
      if (editingVehicle.id && data.vehicule.find(v => v.id === editingVehicle.id)) {
        // Update existing vehicle
        updatedVehicles = data.vehicule.map(v =>
        ...
        updatedVehicles = [...data.vehicule, newVehicle];
      }
      
      onUpdateData({
        ...data,
        vehicule: updatedVehicles
      });
  ...
  const handleDeleteVehicle = (id: string) => {
    if (confirm('Ești sigur că vrei să ștergi acest vehicul?')) {
      const updatedVehicles = data.vehicule.filter(v => v.id !== id);
